...
/**
 * Auditable, minimal content packaging layer for program uploads.
 * Payload layouts are inferred from recovered start-header + chunk transport patterns.
 */
...
object ProgramComposer {
    fun compose(
        ...
        val body = encodeContent(content)
    ...
    private fun encodeContent(content: ProgramContent): ByteArray = when (content) {
        is ProgramContent.Text -> {
