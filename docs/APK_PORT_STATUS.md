...
- Template/content generators.
  ...
  - Dynamic clock/date/weather/time-count/scoreboard-display template builders (the other four `getDataWith*CombineProgram` functions in `CoolledUXUtils.java`) still need APK source/golden-vector confirmation before being marked exact - these are the largest remaining unported functions in the file (~1090, ~372, ~251, and ~607 lines respectively).
