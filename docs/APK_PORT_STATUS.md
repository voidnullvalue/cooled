...
- Template/content generators.
  ...
  - `CoolledUXUtils.getDataWithDateCombineProgram` (date/weather template) is now ported and byte-exact-tested for exactly one physical device size (`deviceRows=16`/`deviceColumns=32`) - see `ProgramContent.DateWeather` / `CoolleduxDateWeatherBytecode.kt` / `DateWeatherTemplateTest.kt`. Every other device size throws rather than guessing at its per-size digit/week bitmap tables (dozens of untraced device-size variants remain - see `docs/APK_REVERSE_ENGINEERING_NOTES.md`).
  - **`CoolledUXUtils.getDataWithTimeCountCombineProgram(...)` (live countdown/count-up display template) is now exactly ported (2026-07-05)**: `ProgramContent.TimeCount` / `CoolleduxProgramBytecode.timeCount(...)` / `TimeCountDigitTables.kt`, golden-vector tested in `TimeCountTemplateTest.kt` against literal tables copied verbatim from source. jadx's rendering of this function's row/column dispatch has the same kind of scrambled control flow documented elsewhere in this file (naive transcription produces an actual infinite loop via reused labels) - resolved from smali, which is linear with zero backward branches.
  - `getDataWithScoreBoardCombineProgram(...)` (scoreboard display template, ~607 lines) is **investigated but not ported** - four independent digit-table dispatch chains plus a different "omit the block entirely when empty" framing convention, and a smali-extraction tooling reliability concern was flagged mid-trace (see `docs/APK_REVERSE_ENGINEERING_NOTES.md`) - scoped out rather than guessed at.
  - The remaining literal digit-bitmap override tables for the clock/date builders (dozens of untraced per-device-size variants) still need APK source/golden-vector confirmation before being marked exact.
...
- **Arabic/Hebrew/Hindi/Thai dispatch is now wired end-to-end (2026-07-05), for both modes**: `MultiLangTextTokenizer.kt` ports `MultiLangTextEmojiParser.getTextEmojiItems`/`getTextEmojiItemsByLanguage` (script-run splitting + ICU `BreakIterator` word segmentation for languageCode ar/iw/hi/th; everything else still uses the plain `TextEmojiTokenizer`), and `TokenGlyphShaper` (`ShapedGlyph.kt`) now branches per character: a lone Arabic/Hebrew/Hindi/Thai character (`ScriptDetection.isArabic/isHebrew/isHindi/isThai`, regardless of the content's own `languageCode` - matching FontUtils.java:11621's per-char `isLanguageSupported` gate exactly) goes through `GlyphRasterizer.drawChar`; a multi-character word segment goes through `GlyphRasterizer.drawString`; everything else is unchanged (`CoolleduxGlyphPipeline.readAndShapeGlyph`). Both rasterized paths share the same rotate/trim tail as a font-table glyph via the newly-extracted `CoolleduxGlyphPipeline.shapeTail`. `CoolleduxStreamText`/`CoolleduxCombineText` now call `ScriptVisualText.getVisualText` then `MultiLangTextTokenizer` instead of the plain tokenizer directly, so both modes get this for free. `AndroidBleTransport.init` now registers `AndroidGlyphRasterizer` as `GlyphRasterizers.active` alongside the other Android backends.
  - **Verified by**: `MultiLangTextTokenizerTest` (real icu4j `BreakIterator`, no fakes - JVM-portable) and `GlyphRasterizerDispatchTest` (a fake `GlyphRasterizer` proving the dispatch itself - single char vs. word segment vs. plain font table - and that the returned bytes flow through word-wrap/centering exactly like a font-table glyph, in both stream and combine mode end to end).
  - **Documented gap, not guessed at**: non-zero `textRotate` (90/180/270) for a multi-character word glyph throws a clear error rather than guessing - `FontBitmapRotation.rotate90Clockwise` only supports square NxN bitmaps, and the APK's own handling of rotating a `text.length()*showHeight`-wide non-square word bitmap could not be pinned down unambiguously from the decompiled sources (see `TokenGlyphShaper`'s class doc). The "emoji_fc_" embedded-placeholder special case inside `getTextEmojiItemsByLanguage` (and the `reverseConsecutiveEmojis` pass that follows it) is also not ported - it belongs to the pre-existing, separately-tracked emoji/image-token gap below, and the reference implementation's own handling of it looks broken besides.
  - **Still genuinely not JVM-testable**: the real Canvas/Typeface pixel output of `AndroidGlyphRasterizer` itself (only on-device/Robolectric can verify that) - see `ArabicDotMatrix.kt`/`ArabicDotMatrixTest.kt` for the one piece of that path (pixel readback) that is pure and JVM-tested, and `AndroidGlyphRasterizer.kt`'s class doc for the rest.
- Still out of scope for either mode: emoji/image tokens from `TextEmojiManagerCoolLEDUX` embedded *inside* ar/iw/hi/th text (see above) - throws a clear error rather than silently producing wrong output. Live-device visual confirmation of the whole pipeline is also still outstanding.
- `ArabicCharDotMatrixGenerator`'s script-detection gates and `getVisualText`/`reverseIfArabic` (bidi/shaping via the same ICU4J version - `72.1` - the APK bundles) are ported, tested, and now wired in: `ScriptDetection.kt`, `ScriptVisualText.kt`.
...
- `CoolledUXUtils.getDataWithTimeCountCombineProgram(...)` (2026-07-05)

## Current priority

Plain-text and emoji/image CoolLEDUX text generation (both mode branches) is now exact and wired end-to-end for every language, including Arabic/Hebrew/Hindi/Thai (the tokenizer dispatch and `GlyphRasterizer` wiring landed 2026-07-05: a lone Arabic/Hebrew/Hindi/Thai character routes through `GlyphRasterizer.drawChar`, a multi-character word segment through `drawString`, both sharing the font-table path's rotate/trim tail; `AndroidBleTransport.init` now registers `AndroidGlyphRasterizer` as the active backend). Only the real Canvas pixel output (on-device/Robolectric-only, unverified here) and non-zero `textRotate` on a multi-character word glyph (documented gap - `FontBitmapRotation` only supports square bitmaps) remain open for that path. CoolLEDU's text pipeline (real, verified, for showHeight in {16,32}/rescale/mirror/text-only) turned out to cover CoolLEDM and CoolLEDUD ("iLedBike") too - confirmed by direct/side-by-side reading, not assumed from naming - so all three families now share it. **Rescale and mirror are now ported (2026-07-05)**: `CoolleduGlyphPipeline`'s rescale reuses the already-verified `FontGlyphRescale.transfer` (the same 15 `transfer<N>FontTo<M>` functions CoolLEDUX's rescale uses), and `isMirror` is `CoolleduxMirror`'s port of `FontUtils.mirror`/`splitBytes`/`addAllSpiltedBytes` (one more confirmed smali-verified jadx control-flow bug found and fixed along the way: a dropped loop back-edge made `mirror`'s stride-2 branch look like a no-loop copy). Still unported for CoolLEDU: its emoji-token path (a completely different reverse-indexed-list lookup, not the same mechanism CoolLEDUX's tokens use). Also still outside CoolLEDU/M/UD's scope: `DeviceFamily.ILEDCLOCK` (its own separate, ~1000-line, not-yet-ported text function) still falls back to the documented placeholder. Display templates: business-hours (exact), clock (default-table case only), date/weather (one device size only), and time-count (exact) are ported; the scoreboard-display template and the remaining per-device-size digit-bitmap override tables for clock/date-weather are not. Remaining highest-value gaps: CoolLEDU's own emoji-token path, iLedClock's own text pipeline, the scoreboard-display template, the per-device-size override tables, and live-device validation across all of it (including, now, the Arabic/Hebrew/Hindi/Thai Canvas rendering specifically).
