...
Plain-text and emoji/image CoolLEDUX text generation (both mode branches) is now exact and wired end-to-end for every language *except* Arabic/Hebrew/Hindi/Thai specifically (confirmed: `MultiLangTextEmojiParser` only special-cases those four - zh-CN, vi, and everything else already use the tokenizer this repo ports). Remaining highest-value gaps: the actual on-device glyph *rasterization* for those four scripts (`ArabicCharDotMatrixGenerator`'s Canvas/Typeface path - genuinely not JVM-portable, and their tokenizer dispatch is tied to this same work, not independently useful yet), text/asset upload byte-exactness for every family except CoolLEDUX (see "Correctness audit" below), and icon/GIF/template byte-exactness for the *original-asset* upload path (as opposed to inline emoji-in-text, which is done) - then live-device visual validation across all of it.
...
**Not yet fixed, tracked for follow-up**: `RememberedDeviceStore` has no real persistence backing (in-memory only, doesn't survive a ViewModel recreation); `TransferStateMachine` is missing the APK's two-tier retry/restart-from-zero structure (robustness gap, not a correctness bug); `buildOtaStartHeader` is currently dead code with the same compressed-vs-uncompressed CRC risk as the fixed program-start bug, latent until OTA upload is wired up; non-CoolLEDUX families' text/asset content encoding (`ProgramContent.encodeContent`'s else-branches) is an explicitly-documented unverified placeholder, not a quick fix - each family has its own real builder in the APK that needs the same reverse-engineering rigor CoolLEDUX got.
