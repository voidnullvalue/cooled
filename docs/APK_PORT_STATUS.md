...
Current branch is approximately **55–65% through the LED-facing port**.

That estimate is intentionally conservative because the core command surface is mostly present, but the content-program side is still the hard part. Sending simple control bytes is easy; matching the original app's generated text/icon/animation payloads is where most remaining risk lives.
...
- Scan-derived matrix dimensions feeding CoolLEDUX text generation.
...
- CoolLEDUX text program generation.
  ...
  - Exact `CoolledUXUtils` text/combine/program byte layout is not fully proven.
- Scan-record parsing.
...
- Original asset support.
  - Extraction/cataloging is present.
  - Icon/animation/emoji/template-specific encoders are not yet implemented.

## Remaining major work

### High risk / high value

1. Port exact CoolLEDUX content-program builders:
...
3. Port icon/image program generation.

4. Port GIF/animation program generation.
...
9. Add local asset selection/picker models for original APK icon/animation/template assets.
...
12. Golden-vector tests after the exact byte layouts are ported.

## Current priority

Continue porting content-program infrastructure, because that is the largest remaining gap between "controls the device" and "matches the original app's LED feature set."