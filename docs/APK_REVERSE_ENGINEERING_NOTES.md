...
- **Missing/unreadable glyph data returns a zero-filled byte array, not an error.** The APK wraps the actual file read in a try/catch that falls back to a same-sized zero array (`CoolLED.reportError(e)` is called for logging/crash-reporting, but the function still returns successfully with blank bytes) whenever `mFontLibFileStream` is null or the read throws. **This is a real divergence from this repo's current behavior**: `CoolleduxFontSourceTest`/`CoolleduxProgramBytecodeParityTest.fontUtilsRejectsMissingFontsAndEmojiInsteadOfGeneratingFallbackBytes` deliberately throws `IllegalStateException` on a missing glyph instead of returning blank bytes, on the reasoning that failing loudly beats silently uploading wrong-looking content. Per the user's explicit "APK behavior is authoritative when it disagrees with this repo" instruction, this is flagged here rather than silently changed: whether to match the APK's silent-blank-glyph behavior or keep the current hard-failure behavior is a real product decision (safety/debuggability vs. exactness), not a bug fix, and should be made deliberately rather than as a side effect of wiring up `getFontByteDataCoolleduxForEmoji`.
