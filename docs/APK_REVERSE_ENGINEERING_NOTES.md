...
- **Not yet ported**: `transfer<N>FontTo<M>` (glyph up/downscaling when a font asset's native size doesn't match `showHeight`), `ArabicCharDotMatrixGenerator` bidi shaping and runtime glyph drawing, `TextEmojiManagerCoolLEDUX` tokenization/emoji image handling, and the mode-dependent list-assembly wrapper (`getFontByteDataCoolleduxForEmoji` itself) that ties all of the above together.
