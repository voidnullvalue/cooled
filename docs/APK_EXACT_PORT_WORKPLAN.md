...
The clone is a native Android LED-controller app. We only port code paths that materially affect bytes sent to the LED device, bytes parsed from the LED device, local content generation, or BLE transfer behavior.

### Port / reimplement

- BLE scan/connect/discover/MTU/notify/write behavior required for the device.
- Frame encoding/decoding, escaping, CRC, LZSS, start headers, chunk framing, retry/timeout behavior, and pacing.
- CoolLEDUX family detection and scan-record metadata used for matrix dimensions, device id, color type, or feature gating.
- CoolLEDUX command builders/parsers: power, brightness, rhythm/music/mic, mirror/rotate, color mode, drive state, timers, countdown, stopwatch, scoreboard, password, device info, OTA version query, and other LED-facing commands.
- Program/content builders for LED output: text, icon, animation, GIF animation, clock/date/frame/temperature/scoreboard/time-count programs.
- Font, emoji, icon, and animation asset readers used to generate LED payload bytes.
- Local state needed to queue, retry, or validate LED transfers.

### Do not port / intentionally omit

- Google Play Services wrappers.
- Firebase/analytics/crash-reporting/telemetry.
- Ads, store/update prompts, rating prompts, marketing screens.
- Cloud account/login behavior unless it directly gates a required LED command path.
- Generic Android support-library scaffolding generated by build tools.
- Any app UI section that does not control LEDs or generate LED content.

If a class contains both platform plumbing and LED behavior, extract only the LED behavior and leave platform glue behind.
